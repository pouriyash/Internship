namespace Exir.Regate
{
    public static class RegateFile
    {
        public static string Build(string name = "", string value = "", bool isRequired = false, long? length = null,string fileType="image")
        {
            ...
            var markup = $@"
                <input type='text' style='position: absolute; pointer-events: none; z-index: -1; opacity: 0;' tabindex='-1' id='file-field-{name}' name='{name}' value='{value}' {(isRequired ? " required='required' " : "")} />
